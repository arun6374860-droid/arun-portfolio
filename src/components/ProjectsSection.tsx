        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredProjects.map((project, index) => (
            <div
              ...
              className="glass-card rounded-3xl p-6 sm:p-8 border border-white/10 hover:border-cyan-500/50 transition-all duration-300 relative group flex flex-col justify-between overflow-hidden shadow-xl"
              style={{
                boxShadow: `0 10px 30px -10px ${project.glowColor}`,
              }}
            >
              {/* Background ambient gradient glow */}
              <div
                className="absolute top-0 right-0 w-72 h-72 rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-opacity pointer-events-none"
                style={{
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mb-8">
                  {project.technologies.map((tech) => (
                    <span
                      key={tech}
                      className="px-2.5 py-1 rounded-lg bg-slate-900/80 text-slate-300 border border-slate-800 text-[11px] font-mono font-medium hover:border-cyan-500/40 hover:text-cyan-300 transition-colors"
                    >
                      {tech}
                    </span>
                  ))}
              ...
              <div className="pt-4 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  {project.liveUrl && (
                    <a
                      ...
                      className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold text-xs shadow-[0_0_20px_rgba(16,185,129,0.35)] hover:shadow-[0_0_25px_rgba(16,185,129,0.5)] hover:scale-[1.03] transition-all flex items-center gap-2 cursor-pointer border border-emerald-300/40"
                ...
                <div className="flex items-center gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-2.5 rounded-xl bg-white/5 hover:bg-cyan-500/20 text-slate-300 hover:text-cyan-300 transition-colors border border-white/10 flex items-center gap-1.5 text-xs font-mono"
