    ...
    <section id="skills" className="py-24 px-4 sm:px-6 lg:px-8 relative z-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10">
          ...
          {/* Category Tabs */}
          <div className="flex flex-wrap items-center gap-1.5 p-1.5 rounded-2xl glass-card border border-white/10 w-full md:w-auto overflow-x-auto shadow-md">
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all whitespace-nowrap cursor-pointer ${
                    isSelected
                      ? 'bg-gradient-to-r from-cyan-500 to-indigo-600 text-white font-semibold shadow-[0_0_15px_rgba(6,182,212,0.4)] scale-105'
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl glass-card text-xs text-white placeholder-slate-400 border border-white/10 focus:border-cyan-400 focus:outline-none focus:ring-1 focus:ring-cyan-400 transition-colors shadow-inner"
            />
          </div>
        </div>
          ...
          {filteredCategories.map((category) => (
            <div key={category.title} className="space-y-4">
              ...
              <div className="flex items-center gap-2.5 pb-2 border-b border-slate-800/80">
                <div className="p-1.5 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 shadow-[0_0_10px_rgba(6,182,212,0.2)]">
                  {iconMap[category.iconName] || <Sparkles className="w-4 h-4" />}
                </div>
                ...
                {category.skills.map((skill) => (
                  ...
                  >
                    <div
                      ...
                      className="glass-card rounded-2xl p-5 border border-white/10 hover:border-cyan-500/50 transition-all duration-300 group flex flex-col justify-between h-full shadow-lg relative overflow-hidden"
                      style={{
                        boxShadow: `0 10px 25px -8px ${skill.color}20`,
                      }}
                    >
                      {/* Subtle background glow */}
                      <div
                        className="absolute top-0 right-0 w-32 h-32 rounded-full blur-2xl opacity-10 group-hover:opacity-25 transition-opacity pointer-events-none"
          ...
          {filteredCategories.length === 0 && (
            <div className="text-center py-16 glass-card rounded-2xl border border-white/10">
              ...
              <button
                ...
                className="mt-3 text-xs text-cyan-400 font-mono hover:underline cursor-pointer"
              >
